...
class SlicingProfile(models.Model):
    ...
    _sql_constraints = [
        ("unique_name", "UNIQUE(name)", "The profile name should be unique!")
    ]
    ...
    def _datadict(self):
        ...
        attr = [
            ...
            and not a.startswith("create_")
            and "lambda" not in a
            and "CONCURRENCY" not in a
            and not a.startswith("write_")
            and not a.startswith("message")
            and not a.startswith("website")
            and not a.startswith("image")
            and "note" not in a
            and not a.startswith("display")
            and not a.startswith("extruders_count")
            and not a.startswith("milling_count")
            and "name" not in a
            and not a.startswith("no_perimeter_")
            ...
            if a != "extruders":
                ...
                for ext in list_extruders:
                    attr_ex = [
                        a_ex
                        for a_ex in dir(ext)
                        if not a_ex.startswith("_")
                        and not a_ex.startswith("id")
                        and "lambda" not in a_ex
                        and "CONCURRENCY" not in a_ex
                        and not a_ex.startswith("create_")
                        and not a_ex.startswith("write_")
                        and "profile" not in a_ex
                        and "note" not in a_ex
                        and not a_ex.startswith("display")
                        and "name" not in a_ex
    ...
    def _validate_extruders_count(self):
        if self.extruders_count != len(self.extruders):
            raise ValidationError(
                "The number of extruders for this profile is different than"
                " the field 'Extruders count'. Make sure you configure them"
                " both well!"
            )
    ...
    allow_empty_layers = fields.Integer(
        ...
        help=(
            "Do not prevent the gcode builder to trigger an exception if a"
            " full layer is empty and so the print will have to start from"
            " thin air afterward."
        ),
        default=0,
    )
    ...
    avoid_crossing_perimeters = fields.Boolean(
        ...
        help=(
            "Optimize travel moves in order to minimize the crossing of"
            " perimeters. This is mostly useful with Bowden extruders which"
            " suffer from oozing. This feature slows down both the print and"
            " the G_code generation."
        ),
        default=False,
    )
    ...
    avoid_crossing_perimeters_max_detour = fields.Integer(
        ...
        help=(
            "The maximum detour length for avoid crossing perimeters. If the"
            " detour is longer than this value, avoid crossing perimeters is"
            " not applied for this travel path. Detour length could be"
            " specified either as an absolute value or as percentage (for"
            " example 50%) of a direct travel path. (mm or % (zero to"
            " disable), default: 0)"
        ),
        default=0,
    )
    ...
    bottom_fill_pattern = fields.Selection(
        ...
        help=(
            "Fill pattern for bottom infill. This only affects the bottom"
            " visible layer, and not its adjacent solid shells. (rectilinear,"
            " monotonicgapfill, monotonic, concentric, concentricgapfill,"
            " hilbertcurve, archimedeanchords, octagramspiral, smooth;"
            " default: monotonic)"
        ),
        default="monotonic",
    )
    ...
    bottom_solid_min_thickness = fields.Float(
        ...
        help=(
            "The number of bottom solid layers is increased above"
            " bottom_solid_layers if necessary to satisfy minimum thickness of"
            " bottom shell. (mm, default: 0)"
        ),
        default=0,
        digits=(1, 3),
    )
    ...
    bridge_angle = fields.Integer(
        string="Bridge angle",
        help=(
            "Bridging angle override. If left to zero, the bridging angle will"
            " be calculated automatically. Otherwise the provided angle will"
            " be used for all bridges. Use 180° for zero angle. (°,"
            " default: 0)"
        ),
        default=0,
    )
    ...
    bridged_infill_margin = fields.Char(
        ...
        help=(
            "This parameter grows the bridged solid infill layers by the"
            " specified mm to anchor them into the part. Put 0 to deactivate"
            " it. Can be a % of the width of the external perimeter. (mm/%,"
            " default: 200%)"
        ),
        default="200%",
    )
    ...
    brim_ears_pattern = fields.Selection(
        ...
        help=(
            "Pattern for the ear. The concentric is the default one. The"
            " rectilinear has a perimeter around it, you can try it if the"
            " concentric has too many problems to stick to the build plate."
            " (concentric, rectilinear; default: concentric)"
        ),
        default="concentric",
    )
    ...
    clip_multipart_objects = fields.Boolean(
        ...
        help=(
            "When printing multi_material objects, this settings will make"
            " Slic3r to clip the overlapping object parts one by the other"
            " (2nd part will be clipped by the 1st,3rd part will be clipped by"
            " the 1st and 2nd etc)."
        ),
        default=True,
    )
    ...
    curve_smoothing_angle_concave = fields.Integer(
        ...
        help=(
            "Minimum (concave) angle at a vertex to enable smoothing (trying"
            " to create a curve around the vertex). 180 : nothing will be"
            " smooth, 0 : all angles will be smoothened. (°, default: 0)"
        ),
        default=0,
    )
    ...
    curve_smoothing_angle_convex = fields.Integer(
        ...
        help=(
            "Minimum (convex) angle at a vertex to enable smoothing (trying to"
            " create a curve around the vertex). 180 : nothing will be smooth,"
            " 0 : all angles will be smoothened. (°, default: 0)"
        ),
        default=0,
    )
    ...
    curve_smoothing_cutoff_dist = fields.Integer(
        ...
        help=(
            "Maximum distance between two points to allow adding new ones."
            " Allow to avoiddistorting long strait areas. 0 to disable. (mm,"
            " default: 2)"
        ),
        default=2,
    )
    ...
    curve_smoothing_precision = fields.Integer(
        ...
        help=(
            "These parameters allow the slicer to smooth the angles in each"
            " layer. The precision will be at least the new precision of the"
            " curve. Set to 0 to deactivate. Note: as it uses the polygon's"
            " edges and only works in the 2D planes, you must have a very"
            " clean or hand_made 3D model. It's really only useful to smoothen"
            " functional models or very wide angles. (mm, default: 0)"
        ),
        default=0,
    )
    ...
    draft_shield = fields.Boolean(
        string="Draft shield",
        help=(
            "If enabled, the skirt will be as tall as a highest printed"
            " object. This is useful to protect an ABS or ASA print from"
            " warping and detaching from print bed due to wind draft."
        ),
        default=False,
    )
    ...
    enforce_full_fill_volume = fields.Boolean(
        ...
        help=(
            "Experimental option which modifies (in solid infill) fill flow to"
            " have the exact amount of plastic inside the volume to fill (it"
            " generally changes the flow from _7% to +4%, depending on the"
            " size of the surface to fill and the overlap parameters, but it"
            " can go as high as +50% for infill in very small areas where"
            " rectilinear doesn't have good coverage). It has the advantage to"
            " remove theover_extrusion seen in thin infill areas, from the"
            " overlap ratio"
        ),
        default=True,
    )
    ...
    ensure_vertical_shell_thickness = fields.Boolean(
        ...
        help=(
            "Add solid infill near sloping surfaces to guarantee the vertical"
            " shell thickness (top+bottom solid layers)."
        ),
        default=False,
    )
    ...
    exact_last_layer_height = fields.Integer(
        ...
        help=(
            "This setting controls the height of last object layers to put the"
            " last layer at the exact highest height possible. Experimental."
        ),
        default=0,
    )
    ...
    external_infill_margin = fields.Char(
        ...
        help=(
            "This parameter grows the top/bottom/solid layers by the specified"
            " mm to anchor them into the part. Put 0 to deactivate it. Can be"
            " a % of the width of the perimeters. (mm/%, default: 150%)"
        ),
        default="150%",
    )
    ...
    external_perimeters_first = fields.Boolean(
        ...
        help=(
            ...
            " one instead of the default inverse order."
        ),
        default=False,
    )
    ...
    external_perimeters_hole = fields.Boolean(
        ...
        help=(
            "Only do the vase trick on the external side. Useful when you only"
            " want to remove seam from screw hole."
        ),
        default=True,
    )
    ...
    external_perimeters_nothole = fields.Boolean(
        ...
        help=(
            "Only do the vase trick on the external side. Useful when the"
    ...
    external_perimeters_vase = fields.Boolean(
        ...
        help=(
            "Print contour perimeters in two circles, in a continuous way,"
            " like for a vase mode. It needs the external_perimeters_first"
            " parameter to work. Doesn't work for the first layer, as it may"
            " damage the bed overwise. Note that it will use min_layer_height"
            " from your hardware setting as the base height (it doesn't start"
            " at 0), so be sure to put here the lowest value your printer can"
            " handle. if it's not lower than two times the current layer"
            " height, it falls back to the normal algorithm, as there is not"
            " enough room to do two loops."
        ),
        default=False,
    )
    ...
    extra_perimeters = fields.Integer(
        ...
        help=(
            "Add more perimeters when needed for avoiding gaps in sloping"
            " walls. Slic3r keeps adding perimeters, until more than 70% of"
            " the loop immediately above is supported. If you succeed in"
            " triggering the algorithm behind this setting, please send me a"
    ...
    extra_perimeters_odd_layers = fields.Boolean(
        ...
        help=(
            "Add one perimeter every odd layer. With this, infill is taken"
            " into the sandwich and you may be able to reduce drastically the"
    ...
    extra_perimeters_overhangs = fields.Boolean(
        ...
        help=(
            "Add more perimeters when needed for avoiding gaps in sloping"
            " walls. Slic3r keeps adding perimeters until all overhangs are"
            " filled. !! this is a very slow algorithm !! If you use this"
            " setting, strongly consider also using overhangs_reverse."
        ),
        default=False,
    )
    ...
    fill_angle = fields.Integer(
        string="Fill angle",
        help=(
            "Default base angle for infill orientation. Cross_hatching will be"
            " applied to this. Bridges will be infilled using the best"
            " direction Slic3r can detect, so this setting does not affect"
            " them. (°, default: 45)"
        ),
        default=45,
    )
    ...
    fill_angle_increment = fields.Integer(
        ...
        help=(
            "Add this angle each layer to the base angle for infill. May be"
            " useful for art, or to be sure to hit every object's feature even"
            " with very low infill. Still experimental, tell me what makes it"
            " useful, or the problems that arise using it. (°, default: 0)"
        ),
        default=0,
    )
    ...
    fill_density = fields.Selection(
        ...
        string="Fill density",
        help=(
            "Density of internal infill, expressed in the range 0% _ 100%. (%,"
            " default: 18%)"
        ),
        default="18%",
    )
    ...
    fill_pattern = fields.Selection(
        ...
        string="Fill pattern",
        help=(
            "Fill pattern for general low_density infill. (rectilinear,"
            " monotonic, grid, triangles, stars, cubic, line, concentric,"
            " honeycomb, 3dhoneycomb, gyroid, hilbertcurve, archimedeanchords,"
            " octagramspiral, scatteredrectilinear, adaptivecubic,"
            " supportcubic; default: stars)"
        ),
        default="stars",
    )
    ...
    fill_smooth_distribution = fields.Char(
        ...
        help=(
            "This is the percentage of the flow that is used for the second"
            " ironing pass. Typical 10_20%. Should not be higher than 20%,"
            " unless you have your top extrusion width greatly superior to"
            " your nozzle width. A value too low and your extruder will eat"
            " the filament. A value too high and the first pass won't print"
            " well. (%, default: 10%)"
        ),
        default="10%",
    )
    ...
    fill_smooth_width = fields.Char(
        ...
        help=(
            "This is the width of the ironing pass, in a % of the top infill"
            " extrusion width, should not be more than 50% (two times more"
            " lines, 50% overlap). It's not necessary to go below 25% (four"
            " times more lines, 75% overlap). If you have problems with your"
            " ironing process, don't forget to look at the flow_>above bridge"
            " flow, as this setting should be set to min 110% to let you have"
            " enough plastic in the top layer. A value too low will make your"
            " extruder eat the filament. (mm/%, default: 50%)"
        ),
        default="50%",
    )
    ...
    first_layer_height = fields.Char(
        ...
        help=(
            ...
            " to print a thicker bottom layer to improve adhesion and"
            " tolerance for non perfect build plates. This can be expressed as"
            " an absolute value or as a percentage (for example: 75%) over the"
            " default nozzle width. (mm or %, default: 75%)"
        ),
        default="75%",
    )
    ...
    first_layer_size_compensation = fields.Integer(
        ...
        help=(
            "The first layer will be grown / shrunk in the XY plane by the"
            " configured value to compensate for the 1st layer squish aka an"
            " Elephant Foot effect. (should be negative = inwards) (mm,"
            " default: 0)"
        ),
        default=0,
    )
    ...
    gap_fill = fields.Boolean(
        string="Gap fill",
        help=(
            "Enable gap fill algorithm. It will extrude small lines between"
            " perimeters when there is not enough space for another perimeter"
            " or an infill."
        ),
        default=True,
    )
    ...
    gap_fill_min_area = fields.Char(
        ...
        help=(
            "This setting represents the minimum mm² for a gapfill extrusion"
            " to be created. Can be a % of (perimeter width)² (default: 100%)"
        ),
        default="100%",
    )
    ...
    hole_size_compensation = fields.Integer(
        ...
        help=(
            "The convex holes will be grown / shrunk in the XY plane by the"
            " configured value (negative = inwards, positive = outwards,"
            " should be negative as the holes are always a bit smaller irl)."
            " This might be useful for fine_tuning hole sizes. This setting"
            " behaves the same as 'Inner XY size compensation' but only for"
            " convex shapes. It's added to 'Inner XY size compensation', it"
            " does not replace it. (mm, default: 0)"
        ),
        default=0,
    )
    ...
    hole_size_threshold = fields.Integer(
        ...
        help=(
            "Maximum area for the hole where the hole_size_compensation will"
            " apply fully. After that, it will decrease down to 0 for four"
            " times this area. Set to 0 to let the hole_size_compensation"
    ...
    hole_to_polyhole = fields.Boolean(
        ...
        help=(
            "Search for almost_circular holes that span more than one layer"
            " and convert the geometry to polyholes. Use the nozzle size and"
            " the (biggest) diameter to compute the polyhole. See"
    ...
    hole_to_polyhole_threshold = fields.Float(
        ...
        help=(
            "Maximum defection of a point to the estimated radius of the"
            " circle. As cylinders are often exported as triangles of varying"
            " size, points may not be on the circle circumference. This"
            " setting allows you some leway to broaden the detection. In mm or"
            " in % of the radius. (mm², default: 0.01)"
        ),
        default=0.01,
        digits=(1, 2),
    )
    ...
    infill_anchor = fields.Char(
        ...
        help=(
            "Connect an infill line to an internal perimeter with a short"
            " segment of an additional perimeter. If expressed as percentage"
            " (example: 15%) it is calculated over infill extrusion width."
            " Slic3r tries to connect two close infill lines to a short"
            " perimeter segment. If no such perimeter segment shorter than"
            " infill_anchor_max is found, the infill line is connected to a"
            " perimeter segment at just one side and the length of the"
            " perimeter segment taken is limited to this parameter, but no"
            " longer than anchor_length_max. Set this parameter to zero to"
    ...
    infill_anchor_max = fields.Integer(
        ...
        help=(
            ...
            " Slic3r tries to connect two close infill lines to a short"
            " perimeter segment. If no such perimeter segment shorter than"
            " this parameter is found, the infill line is connected to a"
            " perimeter segment at just one side and the length of the"
            " perimeter segment taken is limited to infill_anchor, but no"
            " longer than this parameter. If set to 0, the old algorithm for"
            " infill connection will be used, it should create the same result"
            " as with 1000 & 0. (mm or %, default: 0)"
        ),
        default=0,
    )
    ...
    infill_connection = fields.Selection(
        ...
        help=(
            "Give to the infill algorithm if the infill needs to be connected,"
            " and on which perimeters Can be useful for art or with high"
            " infill/perimeter overlap. The result may vary between infill"
            " types. (connected, holes, outershell, notconnected; default:"
            " connected)"
        ),
        default="connected",
    )
    ...
    infill_dense = fields.Boolean(
        string="Infill dense",
        help=(
            "Enables the creation of a support layer under the first solid"
            " layer. This allows you to use a lower infill ratio without"
            " compromising the top quality. The dense infill is laid out with"
            " a 50% infill density."
        ),
        default=False,
    )
    ...
    infill_dense_algo = fields.Selection(
        ...
        help=(
            "Choose the way the dense layer is laid out. The automatic option"
            " lets it try to draw the smallest surface with only strait lines"
            " inside the sparse infill. The Anchored option just slightly"
            " enlarges (by 'Default infill margin') the surfaces that need a"
            " better support. (automatic, autosmall, autoenlarged, enlarged;"
    ...
    infill_every_layers = fields.Integer(
        ...
        help=(
            "This feature allows you to combine infill and speed up your print"
            " by extruding thicker infill layers while preserving thin"
            " perimeters, thus accuracy. (layers, default: 1)"
        ),
        default=1,
    )
    ...
    infill_first = fields.Integer(
        string="Infill first",
        help=(
            " This option will switch the print order of perimeters and"
            " infill, making the latter first."
        ),
        default=0,
    )
    ...
    infill_only_where_needed = fields.Boolean(
        ...
        help=(
            "This option will limit infill to the areas actually needed for"
            " supporting ceilings (it will act as internal support material)."
            " If enabled, this slows down the G_code generation due to the"
    ...
    interface_shells = fields.Boolean(
        ...
        help=(
            "Force the generation of solid shells between adjacent"
            " materials/volumes. Useful for multi_extruder prints with"
    ...
    ironing_type = fields.Selection(
        selection=[
            ("top", "All top surfaces"),
            ("topmost", "Topmost surface only"),
            ("solid", "All solid surfaces"),
        ],
        string="Ironing type",
        help="",
        default="top",
    ...
    model_precision = fields.Float(
        ...
        help=(
            "This is the rounding error of the input object. It's used to"
            " align points that should be in the same line. Put 0 to disable."
            " (mm, default: 0.0001)"
        ),
        default=0.0001,
        digits=(1, 4),
    ...
    no_perimeter_unsupported_algo = fields.Selection(
        ...
        help=(
            "Experimental option to remove perimeters where there is nothing"
            " under them and where a bridged infill should be better. * Remove"
            " perimeters: remove the unsupported perimeters, leave the bridge"
            " area as_is. * Keep only bridges: remove the perimeters in the"
            " bridge areas, keep only bridges that end in solid area. * Keep"
            " bridges and overhangs: remove the unsupported perimeters, keep"
            " only bridges that end in solid area, fill the rest with overhang"
            " perimeters+bridges. * Fill the voids with bridges: remove the"
            " unsupported perimeters, draw bridges over the whole hole.* !!"
            " this one can escalate to problems with overhangs shaped like"
            " /\\, so you should use it only on one layer at a time via the"
            " height_range modifier! !!Computationally intensive!!. (none,"
            " noperi, bridges, bridgesoverhangs, filled; default: none)"
        ),
        default="none",
    )
    ...
    only_one_perimeter_top = fields.Boolean(
        ...
        help=(
            "Use only one perimeter on flat top surface, to give more space to"
            " the top infill pattern."
        ),
        default=True,
    )
    ...
    only_one_perimeter_top_other_algo = fields.Boolean(
        ...
        help=(
            "If you have some problem with the 'Only one perimeter on Top"
            " surfaces' option, you can try to activate this on the"
            " problematic layer."
        ),
        default=False,
    )
    ...
    overhangs_reverse = fields.Boolean(
        ...
        help=(
            "Extrude perimeters that have a part over an overhang in the"
            " reverse direction on odd layers. This alternating pattern can"
            " drastically improve steep overhang. !! this is a very slow"
            " algorithm (it uses the same results as"
    ...
    overhangs_reverse_threshold = fields.Char(
        ...
        help=(
            "Number of mm the overhang need to be for the reversal to be"
            " considered useful. Can be a % of the perimeter width. (default:"
            " 250%)"
        ),
        default="250%",
    )
    ...
    overhangs_width = fields.Char(
        ...
        help=(
            "Minimum unsupported width for an extrusion to apply the bridge"
            " flow to this overhang. Can be in mm or in a % of the nozzle"
            " diameter. Set to 0 to deactivate. (default: 75%)"
        ),
        default="75%",
    )
    ...
    overhangs_width_speed = fields.Char(
        ...
        help=(
            "Minimum unsupported width for an extrusion to apply the bridge"
            " fan & overhang speed to this overhang. Can be in mm or in a % of"
            " the nozzle diameter. Set to 0 to deactivate. (default: 55%)"
        ),
        default="55%",
    )
    ...
    perimeter_bonding = fields.Char(
        ...
        help=(
            "This setting may slightly degrade the quality of your external"
            " perimeter, in exchange for a better bonding between"
            " perimeters.Use it if you have great difficulties with perimeter"
            " bonding, for example with high temperature filaments. This"
            " percentage is the % of overlap between perimeters, a bit like"
            " perimeter_overlap and external_perimeter_overlap, but in"
            " reverse. You have to set perimeter_overlap and"
            " external_perimeter_overlap to 100%, or this setting has no"
            " effect. 0: no effect, 50%: half of the nozzle will be over an"
            " already extruded perimeter while extruding a new one, unless"
            " it's an external one). It's very experimental, please report"
            " about the usefulness. It may be removed if there is no use for"
            " it. (%, default: 0%)"
        ),
        default="0%",
    )
    ...
    perimeter_loop = fields.Boolean(
        ...
        help=(
            "Join the perimeters to create only one continuous extrusion"
            " without any z_hop. Long inside travel (from external to holes)"
            " are not extruded to give some space to the infill."
        ),
        default=False,
    )
    ...
    perimeter_loop_seam = fields.Selection(
        ...
        help=(
            "Position of perimeters starting points. (nearest, rear; default:"
            " rear)"
        ),
        default="rear",
    )
    ...
    perimeter_round_corners = fields.Boolean(
        ...
        help=(
            "Internal perimeters will go around sharp corners by turning"
            " around instead of making the same sharp corner. This can help"
            " when there are visible holes in sharp corners on perimeters"
        ),
        default=False,
    )
    ...
    perimeters = fields.Integer(
        string="Perimeters",
        help=(
            "This option sets the number of perimeters to generate for each"
            " layer. Note that Slic3r may increase this number automatically"
            " when it detects sloping surfaces which benefit from a higher"
    ...
    resolution = fields.Float(
        string="Resolution",
        help=(
            "Minimum detail resolution, used to simplify the input file for"
            " speeding up the slicing job and reducing memory usage."
            " High_resolution models often carry more details than printers"
            " can render. Set to zero to disable any simplification and use"
            " full resolution from input. Note: Slic3r has an internal working"
            " resolutionof 0.0001mm. Infill & Thin areas are simplified up to"
            " 0.0125mm. (mm, default: 0.002)"
        ),
        default=0.002,
        digits=(1, 3),
    )
    ...
    seam_angle_cost = fields.Char(
        ...
        help=(
            "Cost of placing the seam at a bad angle. The worst angle (max"
            " penalty) is when it's flat. (%, default: 100%)"
        ),
        default="100%",
    )
    ...
    seam_position = fields.Selection(
        ...
        help=(
            "Position of perimeters' starting points. (cost, random, aligned,"
            " rear; default: cost)"
        ),
        default="hidden",
    )
    ...
    seam_travel_cost = fields.Char(
        ...
        help=(
            "Cost of moving the extruder. The highest penalty is when the"
            " point is the furthest from the position of the extruder before"
            " extruding the external perimeter (%, default: 100%)"
        ),
        default="100%",
    )
    ...
    solid_fill_pattern = fields.Selection(
        ...
        help=(
            "Fill pattern for solid (internal) infill. This only affects the"
            " solid not_visible layers. You should use rectilinear in most"
            " cases. You can try ironing for translucent material. Rectilinear"
            " (filled) replaces zig_zag patterns by a single big line & is"
            " more efficient for filling little spaces. (smooth, rectilinear,"
            " rectilineargapfill, monotonic, concentric, concentricgapfill,"
            " hilbertcurve, archimedeanchords, octagramspiral; default:"
            " rectilineargapfill)"
        ),
    ...
    solid_infill_below_area = fields.Integer(
        ...
        help=(
            "Force solid infill for regions having a smaller area than the"
            " specified threshold. (mm², default: 70)"
        ),
        default=70,
    )
    ...
    solid_infill_every_layers = fields.Integer(
        ...
        help=(
            "This feature allows you to force a solid layer every given number"
            " of layers.Zero to disable. You can set this to any value (for"
            " example 9999); Slic3r will automatically choose the maximum"
            " possible number of layers to combine according to nozzle"
            " diameter and layer height. (layers, default: 0)"
        ),
        default=0,
    )
    ...
    spiral_vase = fields.Boolean(
        string="Spiral vase",
        help=(
            "This feature will raise Z gradually while printing a"
            " single_walled object in order to remove any visible seam. This"
            " option requires a single perimeter, no infill, no top solid"
            " layers and no support material. You can still set any number of"
            " bottom solid layers as well as skirt/brim loops. It won't work"
    ...
    standby_temperature_delta = fields.Float(
        ...
        help=(
            "Temperature difference to be applied when an extruder is not"
            " active. Enables a full_height 'sacrificial' skirt on which the"
            " nozzles are periodically wiped. (∆°C, default: -5)"
        ),
        default=-5,
        digits=(1, 0),
    )
    ...
    thin_perimeters = fields.Boolean(
        ...
        help=(
            "Allow outermost perimeter to overlap itself to avoid the use of"
            " thin walls. Note that flow isn't adjusted and so this will"
            " result in over_extruding and undefined behavior."
        ),
        default=True,
    )
    ...
    thin_walls = fields.Boolean(
        string="Thin walls",
        help=(
            "Detect single_width walls (parts where two extrusions don't fit"
            " and we need to collapse them into a single trace). If unchecked,"
            " Slic3r may try to fit perimeters where it's not possible,"
            " creating some overlap leading to over_extrusion."
        ),
        default=True,
    )
    ...
    thin_walls_merge = fields.Boolean(
        ...
        help=(
            "Allow the external perimeter to merge the thin walls in the path."
            " You can deactivate this if you are using thin walls as a custom"
            " support, to reduce adhesion a little."
        ),
        default=True,
    )
    ...
    thin_walls_min_width = fields.Char(
        ...
        help=(
            "Minimum width for the extrusion to be extruded (widths lower than"
            " the nozzle diameter will be over_extruded at the nozzle"
            " diameter). If expressed as percentage (for example 110%) it will"
            " be computed over nozzle diameter. The default behavior of"
            " PrusaSlicer is with a 33% value. Put 100% to avoid any sort of"
            " over_extrusion. (default: 33%)"
        ),
        default="33%",
    )
    ...
    thin_walls_overlap = fields.Char(
        ...
        help=(
            "Overlap between the thin wall and the perimeters. Can be a % of"
            " the external perimeter width (default 50%) (default: 50%)"
        ),
        default="50%",
    )
    ...
    top_fill_pattern = fields.Selection(
        ...
        help=(
            "Fill pattern for top infill. This only affects the top visible"
            " layer, and not its adjacent solid shells. (rectilinear,"
            " monotonicgapfill, monotonic, concentric, concentricgapfill,"
            " hilbertcurve, archimedeanchords, octagramspiral, sawtooth,"
            " smooth, smoothtriple, smoothhilbert; default: monotonic)"
        ),
        default="monotonic",
    )
    ...
    top_solid_min_thickness = fields.Float(
        ...
        help=(
            "The number of top solid layers is increased above"
            " top_solid_layers if necessaryto satisfy minimum thickness of top"
            " shell. This is useful to prevent pillowing effect when printing"
            " with variable layer height. (mm, default: 0)"
        ),
        default=0,
        digits=(1, 3),
    )
    ...
    wipe_tower_bridging = fields.Integer(
        ...
        help=(
            "Maximal distance between supports on sparse infill sections. (mm,"
            " default: 10)"
        ),
        default=10,
    )
    ...
    wipe_tower_brim = fields.Char(
        ...
        help=(
            "Width of the brim for the wipe tower. Can be in mm or in % of the"
            " (assumed) only one nozzle diameter. (default: 150%)"
        ),
        default="150%",
    )
    ...
    wipe_tower_rotation_angle = fields.Integer(
        ...
        help=(
            "Wipe tower rotation angle with respect to x_axis. (°, default: 0)"
        ),
        default=0,
    )
    ...
    wipe_tower_x = fields.Integer(
        string="Wipe tower x",
        help=(
            "X coordinate of the left front corner of a wipe tower (mm,"
            " default: 180)"
        ),
        default=180,
    )
    ...
    wipe_tower_y = fields.Integer(
        string="Wipe tower y",
        help=(
            "Y coordinate of the left front corner of a wipe tower (mm,"
            " default: 140)"
        ),
        default=140,
    )
    ...
    xy_inner_size_compensation = fields.Integer(
        ...
        help=(
            "The object will be grown/shrunk in the XY plane by the configured"
            " value (negative = inwards, positive = outwards). This might be"
            " useful for fine_tuning sizes. This one only applies to the"
            " 'inner' shell of the object (!!! horizontal holes break the"
            " shell !!!) (mm, default: 0)"
        ),
        default=0,
    )
    ...
    xy_size_compensation = fields.Integer(
        ...
        help=(
            ...
            " value (negative = inwards, positive = outwards). This might be"
            " useful for fine_tuning sizes. This one only applies to the"
            " 'exterior' shell of the object. !!! it's recommended you put the"
            " same value into the 'Inner XY size compensation', unless you are"
            " sure you don't have horizontal holes. !!! (mm, default: 0)"
        ),
        default=0,
    )
    ...
    external_perimeter_fan_speed = fields.Integer(
        ...
        help=(
            "When set to a non-zero value this fan speed is used only for"
            " external perimeters (visible ones). Set to 1 to disable the fan."
            " Set to -1 to use the normal fanspeed on external"
            " perimeters.External perimeters can benefit from higher fanspeed"
            " to improve surface finish, while internal perimeters, infill,"
            " etc. benefitfrom lower fan speed to improve layer adhesion. (%,"
            " default: -1)"
        ),
        default=-1,
    )
    ...
    filament_cooling_moves = fields.Integer(
        ...
        help=(
            "Filament is cooled by being moved back and forth in the cooling"
            " tubes. Specify desired number of these moves. (default: 4)"
        ),
        default=4,
    )
    ...
    filament_cooling_zone_pause = fields.Integer(
        ...
        help=(
            "Can be useful to avoid bondtech gears deforming hot tips, but not"
            " ordinarily needed (milliseconds, default: 0)"
        ),
        default=0,
    )
    ...
    filament_deretract_speed = fields.Char(
        ...
        help=(
            "The speed for loading of a filament into extruder after"
            " retraction (this only applies to the extruder motor). If left as"
            " zero, the retraction speed is used. (mm/s, default: 0)"
        ),
        default="",
    )
    ...
    filament_diameter = fields.Float(
        ...
        help=(
            "Enter your filament diameter here. Good precision is required, so"
            " use a caliper and do multiple measurements along the filament,"
            " then compute the average. (mm, default: 1.75)"
        ),
        default=1.75,
        digits=(1, 2),
    )
    ...
    filament_enable_toolchange_part_fan = fields.Boolean(
        ...
        help=(
            "Experimental setting. May enable the hotend to cool down faster"
            " during toolchanges (default: 0)"
        ),
        default=False,
    )
    ...
    filament_load_time = fields.Integer(
        ...
        help=(
            "Time for the printer firmware (or the Multi Material Unit 2.0) to"
            " load a new filament during a tool change (when executing the T"
            " code). This time is added to the total print time by the G_code"
    ...
    filament_loading_speed = fields.Integer(
        ...
        help=(
            "Speed used for loading the filament on the wipe tower. (mm/s,"
            " default: 28)"
        ),
        default=28,
    )
    ...
    filament_loading_speed_start = fields.Integer(
        ...
        help=(
            "Speed used at the very beginning of loading phase. (mm/s,"
            " default: 3)"
        ),
        default=3,
    )
    ...
    filament_max_wipe_tower_speed = fields.Integer(
        ...
        help=(
            "This setting is used to set the maximum speed when extruding"
            " inside the wipe tower (use M220). In %, set 0 to disable and use"
            " the Filament type instead. If disabled, these filament types"
            " will have a defaut value of: _ PVA: 80% to 60% _ SCAFF: 35% _"
            " FLEX: 35% _ OTHERS: 100% Note that the wipe tower reset the"
            " speed at 100% for the unretract in any case. If using marlin,"
            " M220 B/R is used to save the speed override before the wipe"
            " tower print. (%, default: 0)"
        ),
        default=0,
    )
    ...
    filament_melt_zone_pause = fields.Integer(
        ...
        help=(
            "Stay in melt zone for this amount of time before extracting the"
            " filament. Not usually necessary. (milliseconds, default: 0)"
        ),
        default=0,
    )
    ...
    filament_minimal_purge_on_wipe_tower = fields.Integer(
        ...
        help=(
            "After a tool change, the exact position of the newly loaded"
            " filament inside the nozzle may not be known, and the filament"
            " pressure is likely not yet stable. Before purging the print head"
            " into an infill or a sacrificial object, Slic3r will always prime"
            " this amount of material into the wipe tower to produce"
    ...
    filament_ramming_parameters = fields.Char(
        ...
        help=(
            "This string is edited by RammingDialog and contains ramming"
            " specific parameters. (default: '120 100 6.6 6.8 7.2 7.6 7.9 8.2"
            " 8.7 9.4 9.9 10.0| 0.05 6.6 0.45 6.8 0.95 7.8 1.45 8.3 1.95 9.7"
            " 2.45 10 2.95 7.6 3.45 7.6 3.95 7.6 4.45 7.6 4.95 7.6')"
        ),
        default=(
            "120 100 6.6 6.8 7.2 7.6 7.9 8.2 8.7 9.4 9.9 10.0| 0.05 6.6 0.45"
            " 6.8 0.95 7.8 1.45 8.3 1.95 9.7 2.45 10 2.95 7.6 3.45 7.6 3.95"
            " 7.6 4.45 7.6 4.95 7.6"
        ),
    )
    ...
    filament_retract_before_travel = fields.Char(
        ...
        help=(
            "Retraction is not triggered when travel moves are shorter than"
            " this length. (mm, default: 2)"
        ),
        default="",
    )
    ...
    filament_retract_before_wipe = fields.Char(
        ...
        help=(
            "With bowden extruders, it may be wise to do some amount of quick"
            " retract before doing the wipe movement. (%, default: 0%)"
        ),
        default="",
    )
    ...
    filament_retract_layer_change = fields.Boolean(
        ...
        help=(
            "This flag enforces a retraction whenever a Z move is done (before"
            " it). (default: 0)"
        ),
        default=False,
    )
    ...
    filament_retract_length = fields.Char(
        ...
        help=(
            "When retraction is triggered, filament is pulled back by the"
            " specified amount (the length is measured on raw filament, before"
            " it enters the extruder). (mm (zero to disable), default: 2)"
        ),
        default="",
    )
    ...
    filament_retract_lift = fields.Char(
        ...
        help=(
            "If you set this to a positive value, Z is quickly raised every"
            " time a retraction is triggered. When using multiple extruders,"
            " only the setting for the first extruder will be considered. (mm,"
            " default: 0)"
        ),
        default="",
    )
    ...
    filament_retract_lift_above = fields.Char(
        ...
        help=(
            ...
            " above the specified absolute Z. You can tune this setting for"
            " skipping lift on the first layers. (mm, default: 0)"
        ),
        default="",
    )
    ...
    filament_retract_lift_below = fields.Char(
        ...
        help=(
            "If you set this to a positive value, Z lift will only take place"
            " below the specified absolute Z. You can tune this setting for"
            " limiting lift to the first layers. (mm, default: 0)"
        ),
        default="",
    )
    ...
    filament_retract_speed = fields.Char(
        ...
        help=(
            "The speed for retractions (this only applies to the extruder"
            " motor). (mm/s, default: 40)"
        ),
        default="",
    )
    ...
    filament_shrink = fields.Char(
        ...
        help=(
            "Enter the shrinkage percentage that the filament will get after"
            " cooling (94% if you measure 94mm instead of 100mm). The part"
            " will be scaled in xy to compensate.Only the filament used for"
            " the perimeter is taken into account. Be sure to allow enough"
            " space between objects, as this compensation is done after the"
            " checks. (%, default: 100%)"
        ),
        default="100%",
    )
    ...
    filament_skinnydip_distance = fields.Integer(
        ...
        help=(
            "For stock extruders, usually 40_42mm. For bondtech extruder"
            " upgrade, usually 30_32mm. Start with a low value and gradually"
            " increase it until strings are gone. If there are blobs on your"
            " wipe tower, your value is too high. (mm, default: 31)"
        ),
        default=31,
    )
    ...
    filament_toolchange_delay = fields.Integer(
        ...
        help=(
            "Time to wait after the filament is unloaded. May help to get"
            " reliable toolchanges with flexible materials that may need more"
            " time to shrink to original dimensions. (s, default: 0)"
        ),
        default=0,
    )
    ...
    filament_toolchange_part_fan_speed = fields.Integer(
        ...
        help=(
            "Experimental setting. Fan speeds that are too high can clash with"
            " the hotend's PID routine. (%, default: 50)"
        ),
        default=50,
    )
    ...
    filament_toolchange_temp = fields.Integer(
        ...
        help=(
            "To further reduce stringing, it can be helpful to set a lower"
            " temperature just prior to extracting filament from the hotend."
            " (°C, default: 200)"
        ),
        default=200,
    )
    ...
    filament_unload_time = fields.Integer(
        ...
        help=(
            "Time for the printer firmware (or the Multi Material Unit 2.0) to"
            " unload a filament during a tool change (when executing the T"
            " code). This time is added to the total print time by the G_code"
            " time estimator. (s, default: 0)"
        ),
        default=0,
    ...
    filament_unloading_speed = fields.Integer(
        ...
        help=(
            "Speed used for unloading the filament on the wipe tower (does not"
            " affect initial part of unloading just after ramming). (mm/s,"
            " default: 90)"
        ),
        default=90,
    )
    ...
    filament_unloading_speed_start = fields.Integer(
        ...
        help=(
            "Speed used for unloading the tip of the filament immediately"
            " after ramming. (mm/s, default: 100)"
        ),
        default=100,
    )
    ...
    filament_use_fast_skinnydip = fields.Boolean(
        ...
        help=(
            "Experimental: drops nozzle temperature during cooling moves"
            " instead of prior to extraction to reduce wait time. (default: 0)"
        ),
        default=False,
    )
    ...
    filament_use_skinnydip = fields.Boolean(
        ...
        help=(
            "Skinnydip performs a secondary dip into the meltzone to burn off"
            " fine strings of filament (default: 0)"
        ),
        default=False,
    )
    ...
    filament_wipe = fields.Boolean(
        ...
        help=(
            ...
            " possible blob on leaky extruders. (default: 0)"
        ),
        default=False,
    )
    ...
    filament_wipe_advanced_pigment = fields.Float(
        ...
        help=(
            ...
            " translucent/natural, 0.2_0.5 for white and 1 for black."
            " (default: 0.5)"
        ),
        default=0.5,
        digits=(1, 1),
    ...
    filament_wipe_extra_perimeter = fields.Char(
        ...
        help=(
            "When the external perimeter loop extrusion ends, a wipe is done,"
            " going slightly inside the print. The number in this settting"
            " increases the wipe by moving the nozzle along the loop again"
            " before the final wipe. (mm, default: 0)"
        ),
        default="",
    )
    ...
    full_fan_speed_layer = fields.Integer(
        ...
        help=(
            ...
            " 'disable_fan_first_layers' to maximum at layer"
            " 'full_fan_speed_layer'. 'full_fan_speed_layer' will be ignored"
            " if lower than 'disable_fan_first_layers', in which case the fan"
            " will be running at maximum allowed speed at layer"
    ...
    color_change_gcode = fields.Text(
        ...
        help=(
            "This G_code will be used as a code for the color change (default:"
            " M600)"
        ),
        default="M600",
    )
    ...
    extra_loading_move = fields.Float(
        ...
        help=(
            ...
            " position during load is exactly the same as it was moved back"
            " during unload. When positive, it is loaded further, if negative,"
            " the loading move is shorter than unloading. (mm, default: _2)"
        ),
        default=-2,
        digits=(1, 0),
    )
    ...
    machine_limits_usage = fields.Selection(
        selection=[
            ("emit_to_gcode", "Also emit limits to G-code"),
            ("time_estimate_only", "Use also for time estimate"),
            ("limits", "Use only as safeguards"),
            ("ignore", "Disable"),
        ],
    ...
    parking_pos_retraction = fields.Integer(
        ...
        help=(
            "Distance of the extruder tip from the position where the filament"
            " is parked when unloaded. This should match the value in printer"
            " firmware. (mm, default: 92)"
        ),
        default=92,
    )
    ...
    pause_print_gcode = fields.Text(
        ...
        help=(
            "This G_code will be used as a code for the pause print (default:"
            " M601)"
        ),
        default="M601",
    )
    ...
    wipe_advanced = fields.Integer(
        ...
        help=(
            "Allow Slic3r to compute the purge volume via smart computations."
            " Use the pigment% of each filament and following parameters"
        ),
        default=0,
    )
    ...
    wipe_advanced_algo = fields.Char(
        ...
        help=(
            "Algorithm for the advanced wipe. Linear : volume = nozzle +"
            " volume_mult * (pigmentBefore_pigmentAfter) Quadratic: volume ="
            " nozzle + volume_mult * (pigmentBefore_pigmentAfter)+ volume_mult"
            " * (pigmentBefore_pigmentAfter)^3 Hyperbola: volume = nozzle +"
            " volume_mult * (0.5+pigmentBefore) / (0.5+pigmentAfter) (linear,"
            " quadra, expo; default: linear)"
        ),
        default="linear",
    )
    ...
    wipe_advanced_multiplier = fields.Integer(
        ...
        help=(
            "The volume multiplier used to compute the final volume to extrude"
            " by the algorithm. (mm3, default: 60)"
        ),
        default=60,
    )
    ...
    wipe_advanced_nozzle_melted_volume = fields.Integer(
        ...
        help=(
            "The volume of melted plastic inside your nozzle. Used by"
            " 'advanced wiping'. (mm3, default: 120)"
        ),
        default=120,
    )
    ...
    z_step = fields.Float(
        string="Z step",
        help=(
            "Set this to the height moved when your Z motor (or equivalent)"
            " turns one step.If your motor needs 200 steps to move your"
            " head/plater by 1mm, this field should be 1/200 = 0.005. Note"
            " that the gcode will write the z values with 6 digits after the"
            " dot if z_step is set (it's 3 digits if it's disabled). Put 0 to"
            " disable. (mm, default: 0.005)"
        ),
        default=0.005,
        digits=(1, 3),
    )
